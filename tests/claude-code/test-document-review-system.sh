...
mkdir -p .specs/memory/docs/specs

# Create a spec document WITH INTENTIONAL ERRORS for the reviewer to catch
cat > .specs/memory/docs/specs/test-feature-design.md <<'EOF'
# Test Feature Design
...
Then review the spec at $TEST_PROJECT/.specs/memory/docs/specs/test-feature-design.md using the criteria from that template.
