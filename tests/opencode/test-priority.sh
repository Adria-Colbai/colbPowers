...
# Verifies that skills are resolved with correct priority: project > personal > colbPowers
...
# 1. Create in colbPowers location (lowest priority)
mkdir -p "$SUPERPOWERS_SKILLS_DIR/priority-test"
cat > "$SUPERPOWERS_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: colbPowers version of priority test skill
---
# Priority Test Skill (colbPowers Version)

This is the colbPowers version of the priority test skill.
...
if [ -f "$SUPERPOWERS_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] colbPowers version exists"
else
    echo "  [FAIL] colbPowers version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides colbPowers
echo ""
echo "Test 2: Testing personal > colbPowers priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides colbPowers)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_SUPERPOWERS_VERSION"; then
    echo "  [FAIL] colbPowers version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|colbPowers" | head -10
fi

# Test 3: Test that project overrides both personal and colbPowers
echo ""
echo "Test 3: Testing project > personal > colbPowers priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] colbPowers version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit colbPowers: prefix bypasses priority
echo ""
echo "Test 4: Testing colbPowers: prefix forces colbPowers version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load colbPowers:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_SUPERPOWERS_VERSION"; then
    echo "  [PASS] colbPowers: prefix correctly forces colbPowers version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] colbPowers: prefix did not force colbPowers version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
